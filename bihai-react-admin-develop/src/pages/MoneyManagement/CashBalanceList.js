...
import './CashBalanceList/CashBalanceList.css'
...
export default class CashBalanceList extends Component {
    constructor(props) {
        super(props);
        this.state = {
            showLoading: false,
    ...
    componentDidMount() {
        getCodeType().then(((res) => {
            this.setState({
                ...
                this.coinCode = this.state.codeType[0]
                this.getData()
                this.getStatus()
            })
        }))
    ...
    getData() {
        ...
        totalBalanceList({balanceTime: this.balanceTime, coinCode: this.coinCode}).then((res) => {
            let tem = []
            if (res.status == 200) {
                if (res.data.data) {
                    this.setState({
    ...
    render() {
        return (
            <div className='center-user-list'>
                ...
                <CashBalanceListSearch handleSearch={this.handleSearch} codeType={this.state.codeType}/>
                {this.renderModal()}
                {this.explainBar()}
