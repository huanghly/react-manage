...
import {
    prohibitStatusList, tradeStatus
...
export default class TradeBlacklist extends Component {
    constructor(props) {
        super(props);
        this.state = {
            listData: null,
    ...
    outB = (e) => {
        //console.log(e)
        tradeStatus({userId: e.userId, isNormalTrade: 1}).then((res) => {
            if (res.status == 200) {
                this.getData()
            } else {
                message.error('失败')
