...
import {
    prohibitStatusList, loginStatus
...
export default class LoginBlackList extends Component {
    constructor(props) {
        super(props);
        this.state = {
            listData: [],
    ...
    outB = (e) => {
        // //console.log(e)
        loginStatus({userId: e.userId, isNormalLogin: 1}).then((res) => {
            if (res.status == 200) {
                this.getData()
            } else {
                message.error('失败')
