...
import {
    prohibitStatusList, tradeStatus, cashFeeStatus
...
export default class CashFeeWhiteList extends Component {
    constructor(props) {
        super(props);
        this.state = {
            listData: null,
    ...
    outB = (e) => {
        //console.log(e)
        cashFeeStatus({userId: e.userId, isCashFee: 1}).then((res) => {
            if (res.status == 200) {
                this.getData()
            } else {
                message.error('失败')
